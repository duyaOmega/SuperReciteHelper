...
当前版本：4.0.1-beta（2026-05-15）
(最新代码在dev分支！main分支是稳定的旧版本。建议使用dev分支的v4.0.1-beta版，效果更好！)
...
仓库中的 SuperReciteHelper.exe 为可执行版本，双击即可使用（Windows）。
...
- pypdf / pymupdf：解析 pdf，该功能我们暂不确定是否仍要支持，后续可能会移除。
...
2. 在导入预览页检查题型和答案，可切换识别方案或手动修改题目。（题库解析需要时间！有数秒无响应是正常现象！）
3. 点击“开始刷题”进入练习。
...
- **尽量少使用Word特殊格式！**（如编号、列表），尽量保持纯文本）。这可以通过全选-剪切-黏贴（仅文本）来实现。
- 使用清晰题号（如 1. / 1、）。
