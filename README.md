...
| 05.27 |根据习概的题库修复了parser.py，同时给PyQt界面补上了根据不同分辨率缩放大小的功能（因为在我的电脑上字实在是太小了）|RavenX2007|
| 06.19 |根据习概的题库修复了parser.py...怎么每次我背思政的时候都能出奇怪bug的...另外ai生成了一个icon|RavenX2007|

# SuperReciteHelper

一个面向日常复习和考试准备的桌面刷题工具。支持**多种文件格式**导入、**自动识别**题型与答案，并根据你的作答情况进行加权抽题，优先复习薄弱点。

当前版本：v5.1（2026-06-19）
(最新代码在dev分支！main分支是稳定的旧版本。建议使用dev分支的v5.1版，效果更好！)
...
1. 准备 Python 环境。作者开发时使用3.14版本，但应该支持绝大部分版本。
2. 安装依赖：
...
## 开发过程
第一份代码由@RavenX2007和ChatGPT共同编写，当时只针对一种格式进行规划，目的是辅助本人背诵。
后续该项目作为本小组程设大作业进行开发。详情请见贡献者名录。
...
## 修改记录
当前版本：v5.1（2026-06-19）
- 5.0.x：由@jih484287-wq主导开发，重构了UI，由tk改为PyQt。
